...
class YoloFormat(pl.LightningDataModule):
    def __init__(self, train_path, val_path, workers, train_transforms, val_transforms, batch_size=None):
       super().__init__()
    ...
    def train_dataloader(self):
        return DataLoader(YoloDataset(transforms=self.train_transforms, path=self.train_path),
                          batch_size=self.batch_size, shuffle=True, num_workers=self.workers,
                          persistent_workers=self.workers > 0, pin_memory=self.workers > 0, collate_fn=collater)
       
    def val_dataloader(self):
        return DataLoader(YoloDataset(transforms=self.val_transforms, path=self.val_path),
                          batch_size=self.batch_size, shuffle=False, num_workers=self.workers,
                          persistent_workers=self.workers > 0, pin_memory=self.workers > 0, collate_fn=collater)
...
if __name__ == '__main__':
    ...
    loader = DataLoader(YoloDataset(transforms=train_transforms, path='D:/train'),
